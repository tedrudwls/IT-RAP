...
class SolverRainbow(object):
    ...
    def train_cmua(self, data_loader, result_dir):
        ...
        print(f"{'='*80}\n")
        return universal_perturbation  # ✅ 추가
    ...
    def train_cmua_with_step_size(self, step_size):
        """
        특정 step size로 CMUA 학습
        TPE optimization을 위한 버전
        
        Args:
            step_size: 테스트할 step size
        
        Returns:
            universal_perturbation: 학습된 universal perturbation
        """
        print(f"\n{'='*50}")
        print(f"Training CMUA with step_size={step_size:.6f}")
        print(f"{'='*50}\n")
        
        # Step size 임시 설정
        original_step_size = self.cmua_step_size
        self.cmua_step_size = step_size
        
        # Training용 data loader 생성
        ...
        train_loader = get_loader(
            ...
            self.config.cmua_batch_size,  # ✅ cmua_batch_size 사용
            'CelebA',
            'train',  # train split
        ...
        # CMUA 학습 (기존 train_cmua 호출)
        universal_perturbation = self.train_cmua(train_loader, self.config.result_dir)
        
        # 원래 값 복원
        self.cmua_step_size = original_step_size
        
        return universal_perturbation


    def evaluate_cmua_success_rate(self, universal_perturbation, num_eval_images=20):
        """
        CMUA의 Success Rate 평가 (TPE objective용)
        
        논문 기준: L2 distance > 0.05이면 성공
        
        Args:
            universal_perturbation: 평가할 perturbation (torch.Tensor)
            num_eval_images: 평가 이미지 수
        
        Returns:
            success_rate: 전체 성공률 (0~1)
        """
        print(f"\n{'='*50}")
        print(f"Evaluating CMUA Success Rate")
        print(f"Evaluation Images: {num_eval_images}")
        print(f"{'='*50}\n")
        ...
        eval_images_processed = 0
        total_success = 0
        total_attempts = 0
        
        # 각 속성별 success 카운트
        transform_success = {attr: 0 for attr in self.selected_attrs}
        transform_total = {attr: 0 for attr in self.selected_attrs}
        
        with torch.no_grad():
            for x_real, c_org, filename in eval_loader:  # ✅ enumerate 제거
                if eval_images_processed >= num_eval_images:
                    break
                
                x_real = x_real.to(self.device)
                
                # Universal perturbation 적용
                x_adv = torch.clamp(x_real + universal_perturbation.to(self.device), 0, 1)
                ...
                for idx, c_trg in enumerate(c_trg_list):
                    ...
                    # 원본 이미지 변환
                    x_fake_orig, _ = self.G(x_real, c_trg)
                    
                    # Adversarial 이미지 변환
                    x_fake_adv, _ = self.G(x_adv, c_trg)
                    
                    # L2 distance 계산
                    l2_error = torch.norm(x_fake_orig - x_fake_adv, p=2).item()
                    
                    # Success 판정 (논문 기준: L2 > 0.05)
                    is_success = l2_error > 0.05
                    
                    # 통계 업데이트
                    attr_name = self.selected_attrs[idx]
                    transform_success[attr_name] += int(is_success)
                    transform_total[attr_name] += 1
                    
                    total_success += int(is_success)
                    total_attempts += 1
                
                eval_images_processed += 1
                
                # 진행 상황 출력
                if eval_images_processed % 5 == 0:
                    current_rate = total_success / total_attempts if total_attempts > 0 else 0
                    print(f"[{eval_images_processed}/{num_eval_images}] "
                        f"Current Success Rate: {current_rate:.2%}")
        ...
        print(f"{'='*50}")
        print(f"Overall Success Rate: {success_rate:.2%} ({total_success}/{total_attempts})")
        print(f"\nPer-Attribute Success Rate:")
        for attr in self.selected_attrs:
            if transform_total[attr] > 0:
                attr_rate = transform_success[attr] / transform_total[attr]
                print(f"  {attr:15s}: {attr_rate:6.2%} ({transform_success[attr]}/{transform_total[attr]})")
        print(f"{'='*50}\n")
        
        # Generator를 다시 train mode로
        self.G.train()

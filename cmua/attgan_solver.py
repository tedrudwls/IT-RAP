...
from attgan_attacks import AttackFunction
...
class SolverRainbow(object):
    ...
    """
    attgan_solver.py에 추가할 CMUA 관련 함수들

    SolverRainbow 클래스에 다음 함수들을 추가하세요:
    """

    

    def train_cmua_with_step_size(self, step_size):
        """
        특정 step_size로 CMUA universal perturbation 학습
        
        Args:
            step_size: PGD attack step size
            
        Returns:
            universal_perturbation: 학습된 universal perturbation (Tensor)
        """
        print(f"\n[CMUA Training] Starting with step_size={step_size:.6f}")
        ...
        train_loader = get_loader(
            ...
            self.config.celeba_crop_size,
            self.config.image_size,
            self.config.cmua_batch_size,  # batch_size
            self.config.dataset,
            mode='train',  # training split 사용
            num_workers=self.config.num_workers,
            start_index=0
        )
        
        # Universal perturbation 초기화
        universal_perturbation = torch.zeros(
            (1, 3, self.config.image_size, self.config.image_size)
        ).to(self.device)
        
        # Momentum buffer
        momentum = torch.zeros_like(universal_perturbation)
        
        # AttackFunction with specified step_size
        attack = AttackFunction(
            config=self.config,
            model=self.G,  # AttGAN Generator
            device=self.device,
            epsilon=self.config.cmua_epsilon,
            a=step_size  # ← NNI가 최적화하는 값
        )
        
        # Training loop
        num_images = min(self.config.cmua_train_images, len(train_loader.dataset))
        processed = 0
        
        print(f"Training on {num_images} images...")
        
        for iteration, (images, labels, filenames) in enumerate(train_loader):
            ...
            # 각 속성에 대해 공격
            for attr_idx in range(labels.size(1)):
                # Target label: flip one attribute
                target_label = labels.clone()
                target_label[0, attr_idx] = 1 - target_label[0, attr_idx]
                
                # Original output
                with torch.no_grad():
                    original_output = self.G(images, target_label)[0]
                
                # Perturbed image
                perturbed_image = torch.clamp(
                    images + universal_perturbation,
                    min=-1, max=1
                )
                perturbed_image.requires_grad_(True)
                
                # Perturbed output  
                perturbed_output = self.G(perturbed_image, target_label)[0]
                
                # Loss
                loss = nn.MSELoss()(perturbed_output, original_output)
                
                # Gradient (autograd.grad만 사용, backward 제거)
                ...
                # Momentum update
                momentum = self.config.cmua_momentum * momentum + grad / (torch.norm(grad, p=1) + 1e-8)
                
                # Universal perturbation update
                universal_perturbation = universal_perturbation + step_size * torch.sign(momentum)
                
                # Clip to epsilon
                universal_perturbation = torch.clamp(
                    universal_perturbation,
                    min=-self.config.cmua_epsilon,
                    max=self.config.cmua_epsilon
                ).detach()
        ...
        print(f"[CMUA Training] Completed!")
        print(f"  Perturbation L_inf: {universal_perturbation.abs().max().item():.6f}")
        print(f"  Perturbation L2: {universal_perturbation.norm(p=2).item():.6f}")
        
        return universal_perturbation


    def evaluate_cmua_success_rate(self, universal_perturbation, num_eval_images=100):
        """
        CMUA universal perturbation의 success rate 평가
        
        Args:
            universal_perturbation: 평가할 perturbation
            num_eval_images: 평가할 이미지 수
            
        Returns:
            success_rate: 전체 success rate (0.0 ~ 1.0)
        """
        print(f"\n[CMUA Evaluation] Evaluating on {num_eval_images} images...")
        ...
        eval_loader = get_loader(
            ...
            self.config.celeba_crop_size,
            self.config.image_size,
            batch_size=1,  # 한 장씩 평가
            dataset=self.config.dataset,
            mode='inference',  # inference split 사용
            num_workers=self.config.num_workers,
            start_index=0
        )
        ...
        for images, labels, filenames in eval_loader:
            ...
            for attr_idx in range(num_attrs):
                ...
                with torch.no_grad():
                    ...
                    original_output = self.G(images, target_label)[0][0]
                    ...
                    perturbed_output = self.G(perturbed_image, target_label)[0][0]
                    
                    diff = original_output - perturbed_output
                    l2_distance = torch.norm(diff, p=2).item() / (diff.numel() ** 0.5)

                    # ← 여기에 디버그 출력 추
                    '''
                    if processed == 1 and attr_idx == 0:  # 첫 번째 이미지, 첫 번째 속성만
                        print(f"[DEBUG] L2 distance: {l2_distance:.6f}, threshold: 0.05")
                        print(f"[DEBUG] Original shape: {original_output.shape}")
                        print(f"[DEBUG] Perturbed shape: {perturbed_output.shape}")
                    if processed <= 3:  # 처음 3개 이미지만
                        print(f"[DEBUG] Image {processed}, Attr {attr_idx}:")
                        print(f"  L2 distance: {l2_distance:.6f}")
                        print(f"  Threshold: 0.05")
                        print(f"  Success: {l2_distance > 0.05}")
                    '''
                    if processed <= 3:
                        print(f"[DEBUG] Image {processed}, Attr {attr_idx}:")
                        print(f"  Original output range: [{original_output.min():.3f}, {original_output.max():.3f}]")
                        print(f"  Perturbed output range: [{perturbed_output.min():.3f}, {perturbed_output.max():.3f}]")
                        print(f"  Image shape: {original_output.shape}")
                        print(f"  L2 distance: {l2_distance:.6f}")
                    # Success criterion: L2 > 0.05 (논문 기준)
                    if l2_distance > 0.05:
            ...
            if processed % 20 == 0:
                current_sr = successful_defenses / total_transformations if total_transformations > 0 else 0
                print(f"  Evaluated: {processed}/{num_eval_images} images, Current SR: {current_sr:.4f}")
        
        # Success rate 계산
        success_rate = successful_defenses / total_transformations if total_transformations > 0 else 0
        
        print(f"\n[CMUA Evaluation] Results:")
    ...
    def train_cmua(self):
        """
        CMUA 최종 학습 (최적 step_size로)
        config.cmua_step_size를 사용
        """
        return self.train_cmua_with_step_size(self.config.cmua_step_size)


    def inference_cmua(self, result_dir):
        """
        CMUA inference - universal perturbation 적용하여 방어 성능 확인
        """
        print(f"\n[CMUA Inference] Loading universal perturbation...")
        
        # Universal perturbation 로드
        pert_path = os.path.join(self.config.model_save_dir, 'cmua_universal_perturbation.pth')
        
        if not os.path.exists(pert_path):
            print(f"❌ Universal perturbation not found: {pert_path}")
            print("   Please run CMUA training first!")
            return
        
        universal_perturbation = torch.load(pert_path, map_location=self.device)
        
        # Evaluation
        success_rate = self.evaluate_cmua_success_rate(
            universal_perturbation,
            num_eval_images=2000  # 전체 inference set
        )
        
        print(f"\n✅ CMUA Inference Complete!")
        print(f"   Success Rate: {success_rate:.4f} ({success_rate*100:.2f}%)")
        ...
        for infer_img_idx, (x_real, c_org, filename) in enumerate(data_loader):
            ...
            for idx, attr_name in enumerate(self.selected_attrs):
                ...
                for step in range(self.max_steps_per_episode):
                    ...
                    with torch.no_grad():
                        original_gen_image = attgan_model.G(x_real, c_trg)
                        perturbed_gen_image = attgan_model.G(perturbed_image, c_trg)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_orig = attgan_model.G(perturbed_image, c_trg)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_jpeg = attgan_model.G(x_adv_jpeg, c_trg)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_opencv = attgan_model.G(x_adv_denoise_opencv, c_trg)
                ...
                with torch.no_grad(): 
                    ...
                    perturbed_gen_image_median = attgan_model.G(x_adv_median, c_trg)
                ...
                with torch.no_grad():
                    ...
                    original_gen_image_padding = attgan_model.G(x_real_padding, c_trg)
                    perturbed_gen_image_padding = attgan_model.G(x_adv_padding, c_trg)
                ...
                with torch.no_grad():
                    ...
                    original_gen_image_transforms = attgan_model.G(x_real_transforms, c_trg)
                    perturbed_gen_image_transforms = attgan_model.G(x_adv_transforms, c_trg)
    ...
    #     transformed_original = attgan_model.G(transformed_x_real, c_trg)
    #     transformed_perturbed = attgan_model.G(transformed_perturbed_image, c_trg)
    ...
    def train_attack(self):
        ...
        for test_img_idx, (x_real, c_org, filename) in enumerate(data_loader):
            ...
            for idx, attr_name  in enumerate(self.selected_attrs):
                ...
                with torch.no_grad():
                    original_gen_image = attgan_model.G(x_real, c_trg)
                    perturbed_gen_image = attgan_model.G(perturbed_image, c_trg) # Initialize perturbed generated image
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_orig = attgan_model.G(perturbed_image, c_trg) # AttGAN version
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_jpeg = attgan_model.G(x_adv_jpeg, c_trg) # AttGAN version
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_opencv = attgan_model.G(x_adv_denoise_opencv, c_trg) # AttGAN version
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_median = attgan_model.G(x_adv_median, c_trg) # AttGAN version
                ...
                with torch.no_grad():
                    ...
                    original_gen_image_padding = attgan_model.G(x_real_padding, c_trg) # AttGAN version
                    perturbed_gen_image_padding = attgan_model.G(x_adv_padding, c_trg) # AttGAN version
                ...
                with torch.no_grad():
                    ...
                    original_gen_image_transforms = attgan_model.G(x_real_transforms, c_trg) # AttGAN version
                    perturbed_gen_image_transforms = attgan_model.G(x_adv_transforms, c_trg) # AttGAN version
